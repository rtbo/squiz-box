...
compress(lzma_stream *strm, FILE *infile, FILE *outfile)
{
	...
	while (true) {
		...
		uint8_t* out_start = strm->next_out;
		const uint8_t* in_start = strm->next_in;

		fprintf(stderr, "action: %s\n", action == LZMA_RUN ? "RUN" : "FINISH");
		fprintf(stderr, "in buf:  %d\n", strm->avail_in);
		...
		read_in += strm->total_in;
		written_out += strm->total_out;

		fprintf(stderr, "code: %s\n", ret == LZMA_STREAM_END ? "STREAM_END" : "OK");
		fprintf(stderr, "in:  %d (total %d) (sum: %d)\n", strm->total_in, read_in, sum_in);
		fprintf(stderr, "out: %d (total %d) (sum: %d)\n", strm->total_out, written_out, sum_out);
		fprintf(stderr, "\n");
