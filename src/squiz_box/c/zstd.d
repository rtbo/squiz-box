...
enum ZSTD_CONTENTSIZE_UNKNOWN = cpp_ulonglong.max;
enum ZSTD_CONTENTSIZE_ERROR = cpp_ulonglong.max - 1;
...
/*======  Helper functions  ======*/
extern (D) auto ZSTD_COMPRESSBOUND(S)(S srcSize)
{
    /* this formula ensures that bound(A) + bound(B) <= bound(A+B) as long as A and B >= 128 KB */
    return ((srcSize) + ((srcSize) >> 8) +
            (((srcSize) < (128 << 10)) ? (((128 << 10) - (srcSize)) >> 11) /* margin, from 64 to 0 */
                 : 0));
}

size_t ZSTD_compressBound(size_t srcSize); /*!< maximum compressed size in worst case single-pass scenario */
uint ZSTD_isError(size_t code); /*!< tells if a `size_t` function result is an error code */
const(char)* ZSTD_getErrorName(size_t code); /*!< provides readable string from an error code */
int ZSTD_minCLevel(); /*!< minimum negative compression level allowed, requires v1.4.0+ */
int ZSTD_maxCLevel(); /*!< maximum compression level available */
int ZSTD_defaultCLevel(); /*!< default compression level, specified by ZSTD_CLEVEL_DEFAULT, requires v1.5.0+ */
...
enum ZSTD_strategy
{
    ...
    btopt = 7,
    btultra = 8,
    btultra2 = 9 /* note : new strategies _might_ be added in the future.
...
alias ZSTD_CStream = ZSTD_CCtx; /**< CCtx and CStream are now effectively same object (>= v1.3.0) */
...
{
    _continue = 0, /* collect more data, encoder decides when to output compressed result, for optimal compression ratio */
    flush = 1, /* flush any data provided so far,
                        * it creates (at least) one new block, that can be decoded immediately on reception;
                        * frame will continue: any future data can still reference previously compressed data, improving compression.
                        * note : multithreaded compression will block to flush as much output as possible. */
    end = 2 /* flush any remaining data _and_ close current frame.
 ...
 */
size_t ZSTD_CStreamInSize(); /**< recommended size for input buffer */
size_t ZSTD_CStreamOutSize(); /**< recommended size for output buffer. Guarantee to successfully flush at least one complete compressed block. */
...
alias ZSTD_DStream = ZSTD_DCtx; /**< DCtx and DStream are now effectively same object (>= v1.3.0) */
...
size_t ZSTD_DStreamInSize(); /*!< recommended size for input buffer */
size_t ZSTD_DStreamOutSize(); /*!< recommended size for output buffer. Guarantee to successfully flush at least one complete block in all circumstances. */
...
extern (D)
auto ZSTD_FRAMEHEADERSIZE_PREFIX(F)(F format)
{
    return format == ZSTD_f_std1 ? 5 : 1;
}

extern (D)
auto ZSTD_FRAMEHEADERSIZE_MIN(F)(F format)
{
    return format == ZSTD_f_std1 ? 6 : 2;
}

enum ZSTD_FRAMEHEADERSIZE_MAX = 18 /* can be useful for static allocation */ ;
enum ZSTD_SKIPPABLEHEADERSIZE = 8;
...
{
    enum ZSTD_WINDOWLOG_MAX = ZSTD_WINDOWLOG_MAX_32;
    enum ZSTD_CHAINLOG_MAX = ZSTD_CHAINLOG_MAX_32;
}
else
{
    enum ZSTD_WINDOWLOG_MAX = ZSTD_WINDOWLOG_MAX_64;
    enum ZSTD_CHAINLOG_MAX = ZSTD_CHAINLOG_MAX_64;
}
...
enum ZSTD_MINMATCH_MAX = 7 /* only for ZSTD_fast, other strategies are limited to 6 */ ;
enum ZSTD_MINMATCH_MIN = 3 /* only for ZSTD_btopt+, faster strategies are limited to 4 */ ;
...
enum ZSTD_STRATEGY_MIN = ZSTD_strategy.fast;
enum ZSTD_STRATEGY_MAX = ZSTD_strategy.btultra2;

enum ZSTD_OVERLAPLOG_MIN = 0;
enum ZSTD_OVERLAPLOG_MAX = 9;

enum ZSTD_WINDOWLOG_LIMIT_DEFAULT = 27; /* by default, the streaming decoder will refuse any frame
...
enum ZSTD_LDM_HASHLOG_MIN = ZSTD_HASHLOG_MIN;
enum ZSTD_LDM_HASHLOG_MAX = ZSTD_HASHLOG_MAX;
enum ZSTD_LDM_MINMATCH_MIN = 4;
enum ZSTD_LDM_MINMATCH_MAX = 4096;
enum ZSTD_LDM_BUCKETSIZELOG_MIN = 1;
enum ZSTD_LDM_BUCKETSIZELOG_MAX = 8;
enum ZSTD_LDM_HASHRATELOG_MIN = 0;
enum ZSTD_LDM_HASHRATELOG_MAX = (ZSTD_WINDOWLOG_MAX - ZSTD_HASHLOG_MIN);

/* Advanced parameter bounds */
enum ZSTD_TARGETCBLOCKSIZE_MIN = 64;
enum ZSTD_TARGETCBLOCKSIZE_MAX = ZSTD_BLOCKSIZE_MAX;
enum ZSTD_SRCSIZEHINT_MIN = 0;
enum ZSTD_SRCSIZEHINT_MAX = int.max;
...
struct ZSTD_compressionParameters
{
    ...
    uint minMatch; /**< match length searched : larger == faster decompression, sometimes less compression */
...
{
    _auto = 0, /* dictionary is "full" when starting with ZSTD_MAGIC_DICTIONARY, otherwise it is "rawContent" */
    rawContent = 1, /* ensures dictionary is always loaded as rawContent, even if it starts with ZSTD_MAGIC_DICTIONARY */
    fullDict = 2 /* refuses to load a dictionary if it does not respect Zstandard's specification, starting with ZSTD_MAGIC_DICTIONARY */
}

enum ZSTD_dictLoadMethod_e
{
    byCopy = 0, /**< Copy dictionary content internally */
    byRef = 1 /**< Reference dictionary content -- the dictionary buffer must outlive its users. */
}

enum ZSTD_format_e
{
    zstd1 = 0, /* zstd frame format, specified in zstd_compression_format.md (default) */
    zstd1_magicless = 1 /* Variant of zstd frame format, without initial 4-bytes magic number.
...
enum ZSTD_dictAttachPref_e
{
     ...
     * this enum.
     */
    dictDefaultAttach = 0, /* Use the default heuristic. */
    dictForceAttach = 1, /* Never copy the dictionary. */
    dictForceCopy = 2, /* Always copy the dictionary. */
    dictForceLoad = 3 /* Always reload the dictionary */
}

enum ZSTD_literalCompressionMode_e
{
    _auto = 0, /**< Automatically determine the compression mode based on the compression level.
                               *   Negative compression levels will be uncompressed, and positive compression
                               *   levels will be compressed. */
    huffman = 1, /**< Always attempt Huffman compression. Uncompressed literals will still be
                               *   emitted if Huffman compression is not profitable. */
    uncompressed = 2 /**< Always emit uncompressed literals. */
}
...
enum ZSTD_paramSwitch_e
{
   ...
   */
    _auto = 0, /* Let the library automatically determine whether the feature shall be enabled */
    enable = 1, /* Force-enable the feature */
    disable = 2 /* Do not use the feature */
}
...
{
    noBlockDelimiters = 0, /* Representation of ZSTD_Sequence has no block delimiters, sequences only */
    explicitBlockDelimiters = 1 /* Representation of ZSTD_Sequence contains explicit block delimiters */
}
 ...
 */
size_t ZSTD_compressSequences(ZSTD_CCtx* cctx, void* dst, size_t dstSize,
 ...
 * with a 4-byte uint, or if the parameter magicVariant is greater than 15 (and therefore invalid).
 *
 * @return : number of bytes written or a ZSTD error.
 */
size_t ZSTD_writeSkippableFrame(void* dst, size_t dstCapacity,
    const(void)* src, size_t srcSize, uint magicVariant);
 ...
 * @return : number of bytes written or a ZSTD error.
 */
size_t ZSTD_readSkippableFrame(void* dst, size_t dstCapacity, uint* magicVariant,
    const(void)* src, size_t srcSize);
 ...
 *  This prototype will generate compilation warnings. */
deprecated("use ZSTD_compress2 with ZSTD_CCtx_loadDictionary")
...
/* Enables rsyncable mode,
    ...
    */
enum ZSTD_c_rsyncable = ZSTD_cParameter.experimentalParam1;

/* Select a compression format.
 * The value must be of type ZSTD_format_e.
 * See ZSTD_format_e enum definition for details */
enum ZSTD_c_format = ZSTD_cParameter.experimentalParam2;

/* Force back-reference distances to remain < windowSize,
 * even when referencing into Dictionary content (default:0) */
enum ZSTD_c_forceMaxWindow = ZSTD_cParameter.experimentalParam3;

/* Controls whether the contents of a CDict
 * are used in place, or copied into the working context.
 * Accepts values from the ZSTD_dictAttachPref_e enum.
 * See the comments on that enum for an explanation of the feature. */
enum ZSTD_c_forceAttachDict = ZSTD_cParameter.experimentalParam4;
 ...
 */
enum ZSTD_c_literalCompressionMode = ZSTD_cParameter.experimentalParam5;

/* Tries to fit compressed block size to be around targetCBlockSize.
 * No target when targetCBlockSize == 0.
 * There is no guarantee on compressed block size (default:0) */
enum ZSTD_c_targetCBlockSize = ZSTD_cParameter.experimentalParam6;

/* User's best guess of source size.
 * Hint is not valid when srcSizeHint == 0.
 * There is no guarantee that hint is close to actual source size,
 * but compression ratio may regress significantly if guess considerably underestimates */
enum ZSTD_c_srcSizeHint = ZSTD_cParameter.experimentalParam7;
 ...
 */
enum ZSTD_c_enableDedicatedDictSearch = ZSTD_cParameter.experimentalParam8;
 ...
 */
enum ZSTD_c_stableInBuffer = ZSTD_cParameter.experimentalParam9;
 ...
 */
enum ZSTD_c_stableOutBuffer = ZSTD_cParameter.experimentalParam10;
 ...
 */
enum ZSTD_c_blockDelimiters = ZSTD_cParameter.experimentalParam11;
 ...
 *
 */
enum ZSTD_c_validateSequences = ZSTD_cParameter.experimentalParam12;
 ...
 */
enum ZSTD_c_useBlockSplitter = ZSTD_cParameter.experimentalParam13;
 ...
 */
enum ZSTD_c_useRowMatchFinder = ZSTD_cParameter.experimentalParam14;
 ...
 */
enum ZSTD_c_deterministicRefPrefix = ZSTD_cParameter.experimentalParam15;
 ...
 */
enum ZSTD_d_format = ZSTD_dParameter.experimentalParam1;
/* ZSTD_d_stableOutBuffer
 * Experimental parameter.
 * Default is 0 == disabled. Set to 1 to enable.
 *
 ...
 */
enum ZSTD_d_stableOutBuffer = ZSTD_dParameter.experimentalParam2;
 ...
 */
enum ZSTD_d_forceIgnoreChecksum = ZSTD_dParameter.experimentalParam3;
 ...
 */
enum ZSTD_d_refMultipleDDicts = ZSTD_dParameter.experimentalParam4;
 ...
 * @return : 0, or an error code (which can be tested using ZSTD_isError()). */
deprecated("use ZSTD_DCtx_setParameter() instead")
 ...
 * This prototype will generate compilation warnings.
 */
deprecated("use ZSTD_CCtx_reset, see zstd.h for detailed instructions")
size_t ZSTD_initCStream_srcSize(ZSTD_CStream* zcs,
    int compressionLevel,
    cpp_ulonglong pledgedSrcSize);
 ...
 * This prototype will generate compilation warnings.
 */
deprecated("use ZSTD_CCtx_reset and ZSTD_CCtx_refCDict, see zstd.h for detailed instructions")
 ...
 */
deprecated("use ZSTD_CCtx_reset, see zstd.h for detailed instructions")
size_t ZSTD_resetCStream(ZSTD_CStream* zcs, cpp_ulonglong pledgedSrcSize);

struct ZSTD_frameProgression
{
    cpp_ulonglong ingested; /* nb input bytes read and buffered */
    cpp_ulonglong consumed; /* nb input bytes actually compressed */
    cpp_ulonglong produced; /* nb of compressed bytes generated and buffered */
    cpp_ulonglong flushed; /* nb of compressed bytes flushed : not provided; can be tracked from caller side */
...
size_t ZSTD_copyCCtx(ZSTD_CCtx* cctx, const(ZSTD_CCtx)* preparedCCtx, cpp_ulonglong pledgedSrcSize); /**<  note: if pledgedSrcSize is not known, use ZSTD_CONTENTSIZE_UNKNOWN */

size_t ZSTD_compressContinue(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const(void)* src, size_t srcSize);
size_t ZSTD_compressEnd(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const(void)* src, size_t srcSize);

/* The ZSTD_compressBegin_advanced() and ZSTD_compressBegin_usingCDict_advanced() are now DEPRECATED and will generate a compiler warning */
deprecated("use advanced API to access custom parameters")
size_t ZSTD_compressBegin_advanced(ZSTD_CCtx* cctx, const(void)* dict, size_t dictSize, ZSTD_parameters params,
    cpp_ulonglong pledgedSrcSize); /**< pledgedSrcSize : If srcSize is not known at init time, use ZSTD_CONTENTSIZE_UNKNOWN */
deprecated("use advanced API to access custom parameters")
size_t ZSTD_compressBegin_usingCDict_advanced(ZSTD_CCtx* cctx, const(ZSTD_CDict)* cdict, ZSTD_frameParameters fParams,
    cpp_ulonglong pledgedSrcSize); /* compression parameters are already set within cdict. pledgedSrcSize must be correct. If srcSize is not known, use macro ZSTD_CONTENTSIZE_UNKNOWN */
/**
...
{
    cpp_ulonglong frameContentSize; /* if == ZSTD_CONTENTSIZE_UNKNOWN, it means this field is not available. 0 means "empty" */
    cpp_ulonglong windowSize; /* can be very large, up to <= frameContentSize */
    uint blockSizeMax;
...
size_t ZSTD_decodingBufferSize_min(cpp_ulonglong windowSize, cpp_ulonglong frameContentSize); /**< when frame content size is not known, pass in frameContentSize == ZSTD_CONTENTSIZE_UNKNOWN */
